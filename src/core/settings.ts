...
export const SCHEMA = {
    ...
    "play.sweepRadius": num({ group: "Play", label: "Sweep radius", def: 1.0, min: 0.3, max: 4, step: 0.1, unit: "m", hint: "How wide a bite the sweep takes. Both ends of the transport share it — the hollow it opens and the heap it lays down are the same size." }),
    "play.sweepDistance": num({ group: "Play", label: "Sweep throw", def: 2.2, min: 0.3, max: 8, step: 0.1, unit: "m", hint: "How far the material is carried. Short next to the radius and the two ends overlap and cancel, which costs amplitude rather than volume — the substrate divides that back out, so the volume asked for is the volume delivered either way." }),
    "play.sweepRate": num({ group: "Play", label: "Sweep rate", def: 0.55, min: 0.05, max: 3, step: 0.05, unit: "m3/s", hint: "Volume carried sideways per second while held. Moves ground whether or not there is anything loose lying on it — the call is made on the CPU, which cannot see the buffer." }),
    "play.ridgeSpeed": num({ group: "Play", label: "Ridge speed", def: 9, min: 1, max: 30, step: 0.5, unit: "m/s", hint: "How fast the wave runs away from you. It does not change the shape it leaves — the stamps are laid every fixed fraction of a radius of travel, not every frame — so this is pace, not amplitude." }),
    "play.ridgeRange": num({ group: "Play", label: "Ridge range", def: 10, min: 2, max: 40, step: 0.5, unit: "m", hint: "How far the wave travels before it dies." }),
    "play.ridgeRadius": num({ group: "Play", label: "Ridge radius", def: 0.9, min: 0.3, max: 4, step: 0.1, unit: "m", hint: "Half-width of the crest. The troughs either side sit at 1.22 of this, where the volume came from — in low-cohesion ground. Snow packs instead of trenching, so they barely appear." }),
    "play.ridgeHeight": num({ group: "Play", label: "Ridge height", def: 0.55, min: 0.05, max: 2.5, step: 0.05, unit: "m", hint: "How tall the crest stands. Solved into a per-stamp depth from the kernel's line integral, so this is metres of crest and not an amplitude that happens to look right. Exact in cohesionless ground; cohesive ground keeps the rim it would otherwise trench, so a snow crest stands about a fifth proud of this." }),
